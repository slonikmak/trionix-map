...
- **Чёткое разделение ответственности**
  ...
  - `TileExecutors` — общий пул потоков для загрузки и декодирования тайлов.
...
- **Зоны для потенциального улучшения**
  ...
  - `TileExecutors` реализован как глобальный статический пул. Для библиотечного кода это удобно, но при интеграции в крупные приложения может понадобиться возможность подменить executor внешним.
...
- **Потенциальные точки оптимизации (по результатам профилирования)**
  ...
  - `TileExecutors` использует фиксированный пул `max(2, availableProcessors)`. В большинстве случаев это разумный компромисс. При специфических нагрузках может потребоваться возможность передать кастомный `ExecutorService`.
